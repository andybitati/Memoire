...
\begin{quote}
Comment concevoir un prototype local multi-agents, exploitable par un analyste, capable de traiter des journaux systemes et reseaux heterogenes, de produire des anomalies candidates et de documenter ses limites experimentales ?
\end{quote}
...
Le memoire repose sur quatre hypotheses prudentes. Premierement, les journaux heterogenes peuvent etre rapproches par un noyau commun de champs sans perdre les donnees originales. Deuxiemement, un routage par famille de logs est plus coherent architecturalement qu'un modele unique applique indistinctement a toutes les sources. Troisiemement, les modeles legers restent utiles lorsqu'ils sont associes a une normalisation correcte, a une correlation et a une validation humaine. Quatriemement, une anomalie non supervisee doit etre consideree comme une piste d'analyse et non comme une preuve directe d'intrusion.

Ces hypotheses sont evaluees par des criteres observables : taux de parsing et conservation des entrees inconnues, compatibilite entre familles de donnees et modeles, metriques supervisees lorsque des labels sont disponibles, latence, consommation de ressources, absence de perte observee dans la campagne Redis et qualite de la tracabilite produite. Lorsque les donnees ou le protocole ne permettent pas une conclusion forte, le resultat est presente comme exploratoire.

\section{Objectif general}
L'objectif general de ce travail est de concevoir et d'evaluer un prototype local multi-agents, appele \logminer, pour la detection d'anomalies candidates dans des journaux systemes et reseaux heterogenes. Le systeme doit couvrir la chaine allant de la collecte des journaux a la visualisation des alertes, en passant par la normalisation, le routage multi-modeles, la detection et la correlation.
...
\begin{enumerate}
  ...
  \item Integrer des mecanismes de detection supervises et non supervises, en distinguant resultats confirmes, exploratoires et anomalies candidates.
...
\begin{itemize}
  ...
  \item une preuve locale Redis de repartition de taches et de reprise apres panne avant acquittement ;
