...
Joseph Mugaruka Chihyoka\IEEEauthorrefmark{1},
...
\begin{figure}[t]
    ...
    \caption{Logical architecture of \logminer{} rendered as a vector PDF. The contribution is the controlled path from heterogeneous inputs to family-aware model selection and analyst-readable incident candidates, not a new standalone detector.}
...
The weights $w_n$, $w_c$ and $w_m$ are fixed policy parameters in the current prototype. They are set empirically to favor high-confidence structural evidence before weaker lexical cues: explicit family columns and dataset-specific fields receive the strongest contribution, marker tokens provide supporting evidence, and source-name evidence is used mainly as a tie-breaker or early hint. The implementation also uses priority ordering and fallback thresholds to avoid brittle decisions when evidence is sparse. Table~\ref{tab:router_weights} reports the exact additive constants used in the current prototype. In practice, routing decisions are not expected to be highly sensitive to small perturbations such as changing the generic column weight from $+8$ to $+10$, because most supported families are separated by multiple simultaneous signals and by fallback priority rules rather than by a single marginal feature. A full sensitivity analysis over these weights is not claimed in this paper; it is included in future work together with the factorial ablation of routing, feature spaces and model choice.
...
In this campaign, the supervisor first processed five different sources before repetition, routed unsupported web/SIEM/cloud formats to fallback, routed syslog to the Linux detector, and reduced sampling only for corrupted unknown input. After three fallback routes, it increased the correlation window from 15 to 20 minutes to favor broader analyst review; the value then remained bounded at 20 minutes during the rest of the campaign. In the current implementation, adaptation is bounded: each decision recomputes parameters from fixed defaults, the correlation window is capped by policy at 20 minutes for repeated fallback routes and 30 minutes for sequential HDFS/BGL-style sources, and sampling is reduced to a lower bounded profile rather than repeatedly divided. Therefore, the present policy does not accumulate an unbounded window increase across cycles. The current telemetry records each decision, reason, selected source, routed family, sample size, window size and status in JSONL bus messages and audit traces, which makes ping-pong behavior between conservative and normal sampling profiles traceable and manually interruptible through the workflow/API control layer. No such sampling oscillation was observed in the 30-cycle campaign. A stricter future version should add an automatic oscillation watchdog, learn policies from analyst feedback and operate over a distributed message bus with back-pressure.
