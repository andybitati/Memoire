...
\title{Logminer: A Lightweight Multi-Agent Framework for Family-Aware Multi-Model Anomaly Detection in Heterogeneous Logs}

\author{\IEEEauthorblockN{Andr\'e Olivier MARHEGANE BITATI}
\IEEEauthorblockA{University of Kinshasa (UNIKIN) \\
Kinshasa, Democratic Republic of the Congo \\
Email: aolivier.marhegane@unikin.ac.cd}}

\maketitle

\begin{abstract}
Security monitoring environments generate heterogeneous logs whose formats, semantics and statistical distributions differ across operating systems, network flows, SIEM exports and distributed systems. Applying a single monolithic detector to all sources may increase confusion between log families and reduce the interpretability of alerts. This paper presents \logminer, a lightweight multi-agent software architecture for anomaly detection in heterogeneous system and network logs. The prototype decomposes the workflow into specialized cooperating agents for collection, parsing, normalization, family-aware routing, anomaly detection, correlation, dashboard visualization and audit. The main contribution is a family-aware multi-model routing strategy: events are normalized into a common schema, assigned to a log family, processed by a specialized lightweight model, then unified as anomaly candidates before being correlated into analyst-readable incidents. Experiments cover Windows logs, Wazuh exports, Linux/auth logs, HDFS, BGL, CICIDS2017 and UNSW/CIC-DDoS data. An ablation compares a monolithic common-feature baseline with family-aware configurations. The results support the feasibility of a local lightweight implementation, while showing that routing should be interpreted as controlled specialization rather than a universal F1 improvement on every dataset. The prototype is positioned as an analytical complement to mature log management and HIDS/SIEM tools, not as a replacement for production SOC platforms.
\end{abstract}

\begin{IEEEkeywords}
anomaly detection, log analysis, multi-agent architecture, cybersecurity, family-aware routing, lightweight machine learning, SIEM complement, log correlation
\end{IEEEkeywords}
...
This paper introduces \logminer, a lightweight multi-agent prototype designed to evaluate a family-aware multi-model strategy for heterogeneous log anomaly detection. The term multi-agent is used here in a software architectural sense: \logminer{} is composed of specialized cooperating agents with distinct functional roles, message exchange and workflow coordination. The prototype does not claim to implement fully autonomous cognitive agents, FIPA-style negotiation, reinforcement-learning cooperation or LLM-agent autonomy. However, it now includes a bounded supervisor agent that performs a perception-state-decision-action cycle for local workflow adaptation.

The contribution of this paper is threefold:
\begin{itemize}
    \item A lightweight multi-agent architecture for log analysis, including collector, parser, normalizer, detector, correlator, dashboard and audit roles.
    \item A family-aware multi-model routing strategy in which normalized logs are assigned to a log family and processed by a specialized model rather than by a single monolithic detector.
    \item A multi-source evaluation covering system logs, SIEM exports and network datasets, including a routing ablation, false positives, quasi-real-time workflow latency, resource consumption and operational limits.
\end{itemize}
...
Before machine learning can be applied, raw logs must be parsed and transformed into usable representations. Template-based approaches such as Drain organize log messages into event templates and enable downstream sequence or statistical modeling \cite{he2016drain}. DeepLog and related approaches model log sequences with deep learning to detect deviations from normal execution patterns \cite{du2017deeplog}. \logminer{} currently uses lightweight parsing and normalization, while template-based parsing is retained as a natural extension for Linux, HDFS and BGL-style logs.
...
\begin{figure}[t]
    ...
    \caption{Logical architecture of \logminer. Heterogeneous logs are collected, parsed, normalized, routed to specialized models, transformed into anomaly candidates and correlated into analyst-readable incidents.}
...
\midrule
Global model & Main baseline: one detector applied to all normalized sources \\
Family-aware models & Measures the effect of specialized detectors by log family \\
Without correlation & Isolates the impact of the routing and detector agents \\
With correlation & Measures the complete pipeline from anomaly candidate to incident \\
Without common normalization & Estimates the cost of heterogeneous feature handling \\
With common normalization & Tests the complete proposed pipeline \\
\bottomrule
\end{tabularx}
\end{table}
...
The correlator is intentionally explainable rather than opaque. It groups anomaly candidates by a configurable temporal window, host, user, source, category, subcategory, protocol and destination port. Incident priority combines event volume, maximum severity, diversity of event identifiers, security category weight and anomaly-score depth. This correlation step improves analyst readability, but it is treated as a pipeline component rather than as the main algorithmic contribution of the paper.

\subsection{Current Software Agents and Autonomy}
The current implementation uses software agents with specialized roles, explicit message exchange and workflow coordination. To move beyond a purely static pipeline, a bounded SupervisorAgent was added. It performs a perception-state-decision-action cycle: it observes available log candidates and local CPU/RAM state, builds a persistent memory, chooses a source and workflow parameters, then launches parsing, routing, detection, correlation and audit. Its policy favors unexplored sources, reduces sampling under uncertainty or repeated errors, increases correlation windows when fallback routing becomes frequent, and records each decision in audit and bus traces. The current decisions are policy-based and explainable rather than learned; therefore, the prototype still does not claim strong cognitive autonomy. Table~\ref{tab:supervisor} summarizes a five-cycle campaign.
...
\begin{figure}[t]
    ...
    \caption{Controlled ablation between a global common-feature model and family-aware models on the same supervised split.}
    \label{fig:ablation}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{False positive rates observed by log family and model.}
    \label{fig:falsepos}
\end{figure}
...
The FastAPI quasi-real-time workflow was executed for five cycles with 8,537 lines per cycle. The mean workflow latency was 10.6473 seconds, with a minimum of 3.0865 seconds and a maximum of 19.8163 seconds. A 30-cycle resource campaign reported a mean workflow duration of 9.3300 seconds and maximum duration of 21.6007 seconds. The API/orchestrator process averaged 187.82 MB RAM and reached 221.20 MB, while the Logminer process averaged 495.41 MB and reached 655.39 MB. Routing itself is based on a bounded sample and a fixed number of family scores; its approximate cost is $O(k \times c \times s)$, where $k$ is the number of families, $c$ the inspected columns and $s$ the sampled rows. A direct profiling run on 8,537 normalized Windows events shows that the router accounts for 0.9\% of the measured routing-detection-correlation time, while model inference accounts for 91.3\% and correlation for 7.7\% (Table~\ref{tab:timing_profile}). These measurements support feasibility on a local machine, but do not establish industrial SOC throughput.
...
\midrule
Multi-agent & Specialized cooperating software agents plus bounded supervisor autonomy, not fully cognitive autonomous agents \\
Distribution & Logical and local service distribution; multi-machine deployment remains future work \\
Adaptive AI & Adaptation by family-aware multi-model routing, not continuous self-learning \\
