...
\title{Logminer: A Family-Aware Multi-Model Routing Architecture for Heterogeneous Log Anomaly Detection}

\author{
\IEEEauthorblockN{G\'ed\'eon Nkishi Djenga\IEEEauthorrefmark{1},
Jean-Pierre Mugisha Muhire\IEEEauthorrefmark{2},
Josu\'e Lungi Selemani\IEEEauthorrefmark{3},
Christian Kutabuna Kubakisa\IEEEauthorrefmark{4},
Joseph Mugaruka Chihyoka\IEEEauthorrefmark{5},
Kyandoghere Kyamakya\IEEEauthorrefmark{6}}
\IEEEauthorblockA{\IEEEauthorrefmark{1}\IEEEauthorrefmark{2}\IEEEauthorrefmark{3}\IEEEauthorrefmark{4}Faculty of Engineering, University of Kinshasa, Congo (DR)\\
Emails: gedeon.nkishi@unikin.ac.cd, tomsmugisha@unikin.ac.cd, josue.lungi@unikin.ac.cd, christian.kutabuna@unikin.ac.cd}
\IEEEauthorblockA{\IEEEauthorrefmark{5}Faculty of Sciences and Technologies, University of Kinshasa, Congo (DR)\\
Email: Mugarukachihyoka@gmail.com}
\IEEEauthorblockA{\IEEEauthorrefmark{6}Institute of Smart Systems Technologies, University of Klagenfurt, Austria\\
...
\begin{abstract}
Security monitoring environments generate heterogeneous logs whose formats, semantics and statistical distributions differ across operating systems, network flows, SIEM exports and distributed systems. Applying a single monolithic detector to all sources may increase confusion between log families and reduce the interpretability of alerts. This paper presents \logminer, a lightweight software-agent architecture for anomaly detection in heterogeneous system and network logs. The prototype decomposes the workflow into specialized cooperating modules for collection, parsing, normalization, family-aware routing, anomaly detection, correlation, dashboard visualization and audit. The main contribution is not a new detector, but a family-aware multi-model routing strategy: events are normalized into a common schema, assigned to a log family with an explicit confidence score, processed by a compatible lightweight model, then unified as anomaly candidates before being correlated into analyst-readable incidents. Experiments cover Windows logs, Wazuh exports, Linux/auth logs, HDFS, BGL, CICIDS2017 and UNSW/CIC-DDoS data. The evaluation includes a controlled common-space ablation and an operational specialization ablation. The results support the feasibility of a local lightweight implementation, while showing that routing should be interpreted as controlled specialization rather than a universal F1 improvement on every dataset. The prototype is positioned as an analytical complement to mature log management and HIDS/SIEM tools, not as a replacement for production SOC platforms.
\end{abstract}
...
\begin{itemize}
    \item A lightweight software-agent architecture for log analysis, including collector, parser, normalizer, router, detector, correlator, dashboard, audit and bounded supervisor roles.
    \item A family-aware multi-model routing strategy in which normalized logs are assigned to a log family through explicit scoring and processed by a compatible specialized model rather than by a single monolithic detector.
    \item A multi-source evaluation covering system logs, SIEM exports and network datasets, including controlled and operational ablations, false positives, quasi-real-time workflow latency, resource consumption, supervisor stability and operational limits.
\end{itemize}

The paper deliberately uses cautious claims. Non-supervised outputs are treated as anomaly candidates, not confirmed intrusions. The validated distribution is mainly logical and modular, while large-scale multi-machine distribution remains future work.
...
Before machine learning can be applied, raw logs must be parsed and transformed into usable representations. Template-based approaches such as Drain organize log messages into event templates and enable downstream sequence or statistical modeling \cite{he2016drain}. DeepLog and related approaches model log sequences with deep learning to detect deviations from normal execution patterns \cite{du2017deeplog}. These methods are powerful for homogeneous sequential logs, but applying the same sequence model indiscriminately to all enterprise sources is technically and economically difficult: Windows events, Linux authentication lines, SIEM exports, cloud JSON records and network flows do not share a single event grammar, sequence length, sampling rhythm or feature contract. A monolithic deep sequence model would therefore require costly global preprocessing, padding or template alignment, and would still risk learning dominant families while weakening minority sources. \logminer{} takes a different route: it first normalizes enough information for audit and routing, then selects a compatible family-specific branch. Lightweight parsing is used in this first version, while template-based parsing is retained as a natural extension for Linux, HDFS and BGL-style logs.
...
The distribution validated in this work is primarily logical and modular. The service-level implementation is partially supported through FastAPI endpoints, a local JSONL bus and an optional Redis-based evolution. A large-scale multi-machine deployment is not claimed as a completed result.
...
Supervisor & Observes sources/resources and adapts workflow parameters & Autonomous bounded control decision \\
\bottomrule
\end{tabularx}
\end{table}

Communication is implemented through file-based artifacts, JSONL messages, FastAPI endpoints and optional Redis evolution. Coordination is handled by an orchestrated workflow, including discovered-source execution and route-based processing.
...
\section{Family-Aware Multi-Model Routing}
...
The novelty is not the isolated use of parsing, machine learning or a dashboard. The novelty is a strategy of controlled specialization: heterogeneous logs are normalized, routed according to family, scored by specialized lightweight models and correlated into incidents. This strategy aims to improve robustness and analyst usability in the presence of heterogeneous formats and distributions; this hypothesis must be evaluated experimentally against monolithic baselines. The claim is therefore architectural and experimental rather than algorithmically theoretical.
...
Factorial routing/features/models & Desired future extension to isolate the independent causal effect of each component \\
\bottomrule
\end{tabularx}
\end{table}
...
\toprule
\textbf{Input} & \textbf{Rows kept} & \textbf{Candidates} & \textbf{Incidents} \\
\midrule
corrupt\_incomplete.log & 2 & 0 & 0 \\
\bottomrule
\end{tabular}
\end{table}

\section{Implementation}
The prototype is implemented in Python with a modular source tree. The stable version uses command-line execution, CSV artifacts and local folders. The service-oriented version exposes core workflow actions through FastAPI, including source discovery, detection and correlation. A local JSONL bus supports message-oriented traces, while Redis is documented as an optional extension.
...
The correlator is intentionally explainable rather than opaque. It groups anomaly candidates by a configurable temporal window, host, user, source, category, subcategory, protocol and destination port. Incident priority combines event volume, maximum severity, diversity of event identifiers, security category weight and anomaly-score depth. For a candidate set $A$, the correlator emits an incident $I$ when candidates share a context key and fall within the window $\Delta t$. The priority score is a weighted combination of candidate count, severity, source diversity and anomaly-score depth. This correlation step improves analyst readability, but it is treated as a pipeline component rather than as the main algorithmic contribution of the paper. The present evaluation reports the resulting incidents and timing overhead; a dedicated correlation-quality evaluation with analyst labels remains future work.

\subsection{Current Software Agents and Bounded Control}
The current implementation uses software agents with specialized roles, explicit message exchange and workflow coordination. To move beyond a purely static script, a bounded SupervisorAgent was added. It performs a perception-state-decision-action cycle: it observes available log candidates and local CPU/RAM state, builds a persistent memory, chooses a source and workflow parameters, then launches parsing, routing, detection, correlation and audit. Its policy favors unexplored sources, reduces sampling under uncertainty or repeated errors, increases correlation windows when fallback routing becomes frequent, and records each decision in audit and bus traces. The current decisions are policy-based and explainable rather than learned; therefore, the supervisor is evaluated as bounded workflow control, not as adaptive artificial intelligence. Table~\ref{tab:supervisor} summarizes a five-cycle campaign.

\begin{table}[t]
\caption{Bounded autonomous SupervisorAgent campaign}
\label{tab:supervisor}
\centering
\scriptsize
\begin{tabular}{r l l l}
\toprule
\textbf{Cycle} & \textbf{Selected source} & \textbf{Routed family} & \textbf{Decision} \\
\midrule
1 & cef.log & fallback & sample=1000, window=15 \\
2 & cloudtrail.jsonl & fallback & sample=1000, window=15 \\
3 & apache\_access.log & fallback & sample=1000, window=15 \\
4 & linux\_auth.log & linux & sample=1000, window=20 \\
5 & corrupt\_incomplete.log & fallback & sample=300, window=20 \\
\bottomrule
\end{tabular}
\end{table}

In this campaign, the supervisor processed five different sources before repetition, routed unsupported web/SIEM/cloud formats to fallback, routed syslog to the Linux detector, and reduced sampling for corrupted unknown input. After three fallback routes, it increased the correlation window from 15 to 20 minutes to favor broader analyst review. A stricter future version should learn policies from analyst feedback and operate over a distributed message bus with back-pressure.

The five-cycle campaign is a functional stability check, not a long-term proof. In the current implementation, adaptation is bounded: each decision recomputes parameters from fixed defaults, the correlation window is capped by policy at 20 minutes for repeated fallback routes and 30 minutes for sequential HDFS/BGL-style sources, and sampling is reduced to a lower bounded profile rather than repeatedly divided. Therefore, the present policy does not accumulate an unbounded window increase across cycles. However, longer campaigns are still required to evaluate drift, repeated fallback behavior and possible oscillations under changing workloads.

\begin{table}[t]
\caption{SupervisorAgent stability indicators}
\label{tab:supervisor_metrics}
\centering
\scriptsize
\begin{tabular}{l l}
\toprule
...
Families observed & fallback, linux \\
Parameter adaptations & 2 \\
Mean cycle duration & 5.8199 s \\
...
The evaluation reports F1-score where labels are available, false positive rates where confusion matrices are available, workflow latency and CPU/RAM snapshots. For unlabeled SIEM exports and operational logs, results are interpreted as anomaly candidates rather than confirmed intrusions.
...
The FastAPI quasi-real-time workflow was executed for five cycles with 8,537 lines per cycle. The mean workflow latency was 10.6473 seconds, with a minimum of 3.0865 seconds and a maximum of 19.8163 seconds. A 30-cycle resource campaign reported a mean workflow duration of 9.3300 seconds and maximum duration of 21.6007 seconds. The API/orchestrator process averaged 187.82 MB RAM and reached 221.20 MB, while the Logminer process averaged 495.41 MB and reached 655.39 MB. This memory footprint is lightweight compared with a full ELK-style stack, but it is still significant for direct installation on production endpoints. The intended deployment for this version is therefore a local analysis workstation or intermediate collection gateway; endpoint hosts may run lightweight collectors or export logs, while model inference and correlation are centralized. Routing itself is based on a bounded sample and a fixed number of family scores; its approximate cost is $O(k \times c \times s)$, where $k$ is the number of families, $c$ the inspected columns and $s$ the sampled rows. A direct profiling run on 8,537 normalized Windows events shows that the router accounts for 0.9\% of the measured routing-detection-correlation time, while model inference accounts for 91.3\% and correlation for 7.7\% (Table~\ref{tab:timing_profile}). These measurements support feasibility on a local machine or gateway, but do not establish industrial SOC throughput.
...
Asynchronism & Current FastAPI/JSONL workflow is mostly synchronous; Kafka, MQTT or Redis Streams would be required for back-pressure and worker parallelism \\
Deployment & Current prototype targets an analysis workstation or intermediate gateway, not a heavy detector installed on every endpoint \\
...
This paper presented \logminer, a lightweight family-aware routing architecture for heterogeneous log anomaly detection. Its main contribution is a multi-model specialization strategy orchestrated by specialized software modules and completed by common normalization, anomaly candidate unification and interpretable correlation. Results across Windows, Wazuh, Linux/auth, HDFS, BGL, CICIDS2017 and UNSW/CIC-DDoS support the feasibility of the approach on a local infrastructure.

The ablation study refines the contribution: the family-aware strategy should be understood as controlled specialization rather than a guarantee of better F1 on every source. Further work includes a factorial ablation separating routing, features and model choice; stronger template-based parsing; routing HDFS/BGL-style logs toward a Drain-like semantic parsing agent followed by temporal or LSTM-style sequence detection; multi-machine deployment; active learning from analyst decisions; learned supervisor policies; longer supervisor stability campaigns; richer incident explanations; and integration with mature SIEM/HIDS platforms.

\balance
\begin{thebibliography}{00}
\bibitem{denning1987intrusion} D. E. Denning, ``An intrusion-detection model,'' \emph{IEEE Transactions on Software Engineering}, vol. SE-13, no. 2, pp. 222--232, 1987.
\bibitem{axelsson2000intrusion} S. Axelsson, ``Intrusion detection systems: A survey and taxonomy,'' Technical Report, 2000.
\bibitem{sommer2010outside} R. Sommer and V. Paxson, ``Outside the closed world: On using machine learning for network intrusion detection,'' in \emph{IEEE Symposium on Security and Privacy}, 2010, pp. 305--316.
\bibitem{chandola2009anomaly} V. Chandola, A. Banerjee, and V. Kumar, ``Anomaly detection: A survey,'' \emph{ACM Computing Surveys}, vol. 41, no. 3, pp. 1--58, 2009.
\bibitem{buczak2016survey} A. L. Buczak and E. Guven, ``A survey of data mining and machine learning methods for cyber security intrusion detection,'' \emph{IEEE Communications Surveys and Tutorials}, vol. 18, no. 2, pp. 1153--1176, 2016.
\bibitem{khraisat2019survey} A. Khraisat, I. Gondal, P. Vamplew, and J. Kamruzzaman, ``Survey of intrusion detection systems: Techniques, datasets and challenges,'' \emph{Cybersecurity}, vol. 2, no. 20, 2019.
\bibitem{he2016drain} P. He, J. Zhu, Z. Zheng, and M. R. Lyu, ``Drain: An online log parsing approach with fixed depth tree,'' in \emph{IEEE International Conference on Web Services}, 2017, pp. 33--40.
\bibitem{du2017deeplog} M. Du, F. Li, G. Zheng, and V. Srikumar, ``DeepLog: Anomaly detection and diagnosis from system logs through deep learning,'' in \emph{ACM CCS}, 2017, pp. 1285--1298.
\bibitem{balasubramaniyan1998aafid} J. S. Balasubramaniyan, J. O. Garcia-Fernandez, D. Isacoff, E. Spafford, and D. Zamboni, ``An architecture for intrusion detection using autonomous agents,'' in \emph{Annual Computer Security Applications Conference}, 1998, pp. 13--24.
\bibitem{nist80092} National Institute of Standards and Technology, ``Guide to computer security log management,'' NIST Special Publication 800-92, 2006.
\bibitem{rfc5424} R. Gerhards, ``The Syslog Protocol,'' RFC 5424, 2009.
\end{thebibliography}
