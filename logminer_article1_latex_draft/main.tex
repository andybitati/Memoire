...
H5 & The prototype is feasible on a local workstation or gateway & Supported by local latency and memory results; SOC-scale throughput is not claimed \\
\bottomrule
\end{tabularx}
\end{table}
...
Fallback stress & Unsupported CEF, CloudTrail and Apache-style sources route to fallback in the supervisor campaign & The fallback branch is used as graceful degradation, not as a high-confidence intrusion claim \\
Timing profile & Router accounts for 0.9\% of routing-detection-correlation time on 8,537 Windows events & The routing layer is not the dominant local bottleneck in the measured workflow \\
\bottomrule
\end{tabularx}
\end{table}
