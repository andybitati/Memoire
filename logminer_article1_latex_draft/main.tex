...
Kyandoghere Kyamakya\IEEEauthorrefmark{2}
}
...
\IEEEauthorblockA{
josue.lungi@unikin.ac.cd, christian.kutabuna@unikin.ac.cd, Mugarukachihyoka@gmail.com, kyandoghere.kyamakya@aau.at
}
}
\maketitle
...
The prototype is positioned as an analytical complement to mature log management and HIDS/SIEM tools rather than as a replacement for production SOC platforms.
\end{abstract}
...
A full sensitivity analysis over these weights is not claimed in this paper; it is included in future work together with the factorial ablation of routing, feature spaces and model choice.
...
\centering
\scriptsize
\begin{tabularx}{\linewidth}{p{2.1cm}r X}
\toprule
\textbf{Evidence} & \textbf{Weight} & \textbf{Use in current router} \\
\midrule
Matched family column & $+8$ each & Generic column evidence for supported families \\
Generic family marker & $+35$ & Marker token found in sampled values or dataset fields \\
Family-specific feature hit & $+10$ to $+12$ each & Windows, Linux, HDFS, BGL and network evidence bundles \\
Dense network profile & $+20$ & Numeric tabular profile typical of network-flow data \\
Filename hint & $+45$ & CICIDS or Linux/auth source-name evidence \\
Specialized dataset bundle & $+70$ to $+120$ & Linux/auth, Wazuh and normalized SIEM evidence \\
Known unsupported subtype & $+140$ fallback & Apache, CEF/LEEF, CloudTrail or unknown subtype \\
Corrupted input marker & $+120$ fallback & Corrupt, incomplete or broken input indicators \\
Syslog subtype & $+140$ linux, $-40$ windows & Prevents syslog batches from being routed to Windows \\
\bottomrule
\end{tabularx}
\end{table}
...
The current local buffering strategy is intentionally simple. Collectors and parsers persist raw files, manifests, normalized records and bus messages as local file artifacts before detection. The detector then processes bounded samples or prepared batches rather than keeping an unbounded in-memory queue. If detection lags behind collection, the backlog remains visible in files and JSONL traces, but the prototype does not yet implement production-grade queue admission, back-pressure, retry policies or priority scheduling. This design is acceptable for a local analytical prototype, but it is not sufficient for sustained SOC ingestion under bursty or adversarial traffic; Kafka, MQTT, Redis Streams or an equivalent broker would be needed for durable asynchronous scaling.
...
Mean cycle duration & 6.3164 s \\
Maximum cycle duration & 11.1918 s \\
...
In this campaign, the supervisor first processed five different sources before repetition, routed unsupported web/SIEM/cloud formats to fallback, routed syslog to the Linux detector, and reduced sampling only for corrupted unknown input. After three fallback routes, it increased the correlation window from 15 to 20 minutes to favor broader analyst review; the value then remained bounded at 20 minutes during the rest of the campaign. In the current implementation, adaptation is bounded: each decision recomputes parameters from fixed defaults, the correlation window is capped by policy at 20 minutes for repeated fallback routes and 30 minutes for sequential HDFS/BGL-style sources, and sampling is reduced to a lower bounded profile rather than repeatedly divided. Therefore, the present policy does not accumulate an unbounded window increase across cycles.

Under prolonged stress, for example a multi-hour stream of corrupted logs, this bounded behavior protects resources but does not solve the observability problem by itself. Repeated fallback routing, maximal windows and minimal sampling would be visible in the audit trail, but they could still create a blind spot if treated as normal operation. The current policy should therefore be interpreted as graceful degradation rather than attack-resistant control. The current telemetry records each decision, reason, selected source, routed family, sample size, window size and status in JSONL bus messages and audit traces, which makes ping-pong behavior between conservative and normal sampling profiles traceable and manually interruptible through the workflow/API control layer. No such sampling oscillation was observed in the 30-cycle campaign. A stricter future version should add an automatic oscillation watchdog, fallback-rate alerts, bounded degradation budgets, source quarantine rules, analyst notification and operation over a distributed message bus with back-pressure.
