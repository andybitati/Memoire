...
Les décisions de l'analyste sont conservées et peuvent influencer certains traitements, mais elles ne déclenchent pas encore une boucle complète de réentraînement. &
Maintenir le réentraînement, la surveillance de la dérive conceptuelle et la promotion de nouveaux modèles parmi les perspectives de recherche.
\tabularnewline
...
Enfin, l'évolution vers l'apprentissage continu doit rester encadrée.

La mémoire adaptative peut servir de point de départ, mais elle ne devrait pas conduire à un réentraînement automatique sans contrôle.

Une architecture plus mature pourrait surveiller certains indicateurs de dérive conceptuelle et signaler lorsqu'un réentraînement semble nécessaire.

Un nouveau modèle serait alors entraîné hors ligne à partir de données validées.

Avant son remplacement, il devrait être comparé au modèle actuellement utilisé sur un jeu de validation gelé, à l'aide de métriques définies à l'avance.

La nouvelle version ne serait déployée que si elle respecte les critères retenus. L'ancienne version devrait rester disponible afin de permettre un retour arrière en cas de dégradation.

Une telle évolution conserverait la logique générale défendue dans ce mémoire : automatiser ce qui peut l'être, tout en maintenant les décisions importantes sous contrôle et en conservant une trace vérifiable des changements effectués.
