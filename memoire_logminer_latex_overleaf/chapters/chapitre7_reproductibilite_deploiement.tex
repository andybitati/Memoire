...
Le prototype repose principalement sur Python. Les versions précises des bibliothèques peuvent évoluer avec le dépôt, mais les principales dépendances concernent le traitement des données, l'apprentissage automatique, l'exposition de services web et la génération des artefacts expérimentaux.
...
Les décisions prises par l'analyste peuvent également devenir une ressource pour les versions futures du système.

Lorsqu'elles sont conservées de manière structurée, elles peuvent servir à identifier les motifs fréquemment rejetés, à recalibrer certains seuils ou à constituer progressivement un ensemble de données destiné à une boucle d'apprentissage actif.

Toute exploitation de ces retours devra cependant rester contrôlée afin d'éviter qu'une décision ponctuelle entraîne automatiquement une modification non validée du comportement des modèles.
...
Chaque modification importante devrait idéalement être associée à une procédure de vérification.

Lorsqu'un parseur est modifié, plusieurs formats connus ainsi que quelques cas limites devraient être testés.

Lorsqu'un modèle est remplacé, les performances de l'ancienne et de la nouvelle version devraient être comparées dans les mêmes conditions.

Lorsqu'une figure ou un tableau est régénéré, les valeurs utilisées dans le mémoire devraient également être contrôlées.
