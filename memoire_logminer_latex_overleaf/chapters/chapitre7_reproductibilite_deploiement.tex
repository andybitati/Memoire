...
La campagne Redis d'endurance de six heures a ete lancee depuis l'hote Windows du projet, dans le dossier \texttt{E:\textbackslash Cours\textbackslash TFE}, avec l'environnement Python local \texttt{.venv}. Le service Redis utilise par le script etait le conteneur Docker \texttt{logminer-redis}, base sur l'image \texttt{redis:7-alpine}, expose sur \texttt{localhost:6379}. L'URL Redis effective etait donc \texttt{redis://localhost:6379/0}.

Deux machines virtuelles VirtualBox etaient lancees pendant l'experience : \texttt{Debian} et \texttt{Ubuntu}. Elles etaient configurees en NAT simple et ont servi a documenter le contexte de preparation d'une extension multi-machine. Toutefois, le bus Redis effectivement utilise par la campagne de six heures etait local a l'hote. La conclusion experimentale doit donc rester precise : le memoire valide une distribution locale multi-processus, une endurance de six heures et une reprise repetee des taches non acquittees ; il ne revendique pas encore une validation SOC multi-machine securisee.
...
Redis Streams et MQTT ont ete explores comme extensions operationnelles locales. Redis Streams a ete valide sur une campagne locale multi-processus de six heures : 8 508 taches uniques terminees, aucun echec, aucun pending final et 709 pannes volontaires avant acquittement recuperees par un worker de reprise. MQTT reste une piste de communication legere entre composants. Ces extensions demandent encore des tests multi-machine, des mecanismes de securisation et une supervision plus stricte avant tout usage operationnel.
...
Les decisions analyste constituent aussi une source de valeur pour l'avenir. Si elles sont conservees proprement, elles peuvent alimenter une boucle d'apprentissage actif. Le systeme pourrait progressivement ajuster ses seuils, reduire les faux positifs ou proposer des priorites plus pertinentes selon l'historique des validations.
...
Plusieurs tests complementaires sont recommandes pour prolonger le travail. Un premier test de charge prolonge a deja ete realise sur Redis Streams pendant six heures, afin de mesurer la stabilite de la repartition locale multi-processus et de la reprise de taches non acquittees. Les prochaines extensions devraient couvrir un test multi-machine, avec plusieurs collecteurs et un bus central, puis des tests de panne plus proches d'un deploiement operationnel, ou un agent est volontairement arrete puis relance sous supervision.
