param(
    ...
    [string]$TaskStream = ("logminer:agent_tasks:vbox_recovery:" + (Get-Date -Format "yyyyMMddHHmmss")),
    [string]$Group = "logminer-intelligent-agents",
    [string]$RunId = ("redis-vbox-recovery-" + (Get-Date -Format "yyyyMMddHHmmss")),
    [string]$DebianUser = "vboxuser",
    [string]$DebianPassword = "",
    [string]$UbuntuUser = "andy",
    [string]$UbuntuPassword = "",
    [string]$OutputJson = "data/processed/vbox_redis_recovery_campaign.json",
    [string]$OutputMarkdown = "docs/memoire/tables/table_vbox_redis_recovery_campaign.md"
)
...
function Invoke-VBoxGuest {
    ...
    if ($AcceptedExitCodes -notcontains $LASTEXITCODE) {
        throw "Commande invitee echouee sur $VmName avec le code $LASTEXITCODE"
    }
}
...
Invoke-Checked "Enfilement des taches" {
...
$debianCrashCommand = "cd `$HOME && LOGMINER_REDIS_URL='$GuestRedisUrl' `$HOME/.venv-logminer/bin/python scripts/logminer_intelligent_agent_worker.py --redis-url '$GuestRedisUrl' --event-stream logminer:events --task-stream '$TaskStream' --group '$Group' --run-id '$RunId' --consumer debian-crash-worker --agent-id debian-crash-worker --crash-after-fetch --max-parallel-tasks 1 --claim-idle-ms 0 --memory data/processed/debian-crash-worker-memory.json; test `$? -eq 2"
$ubuntuRecoveryCommand = "cd `$HOME && LOGMINER_REDIS_URL='$GuestRedisUrl' python3 scripts/logminer_intelligent_agent_worker.py --redis-url '$GuestRedisUrl' --event-stream logminer:events --task-stream '$TaskStream' --group '$Group' --run-id '$RunId' --consumer ubuntu-recovery-worker --agent-id ubuntu-recovery-worker --cycles 6 --max-parallel-tasks 1 --claim-idle-ms 1 --memory data/processed/ubuntu-recovery-worker-memory.json"

Invoke-VBoxGuest -VmName Debian -User $DebianUser -Password $DebianPassword -Command $debianCrashCommand -AcceptedExitCodes @(0, 2)
Start-Sleep -Milliseconds 150
Invoke-VBoxGuest -VmName Ubuntu -User $UbuntuUser -Password $UbuntuPassword -Command $ubuntuRecoveryCommand
...
crashes = [m for m in messages if m.message_type == "agent.simulated_crash"]
...
for message in crashes:
    for task in message.payload.get("fetched_tasks", []):
        task_id = task.get("task_id")
        if task_id:
            crashed_tasks.append(task_id)
recovered_crashed_tasks = len(set(crashed_tasks) & set(unique_tasks))
...
summary = {
    ...
    "simulated_crashes": len(crashes),
    "crashed_tasks": len(set(crashed_tasks)),
    "recovered_crashed_tasks": recovered_crashed_tasks,
...
md = [
    ...
    f"| Taches prises avant panne | {summary['crashed_tasks']} |",
    f"| Taches de panne reprises | {summary['recovered_crashed_tasks']} |",
    f"| Pending final Redis | {summary['pending_after']} |",
    f"| Taches Ubuntu recovery | {summary['by_agent'].get('ubuntu-recovery-worker', 0)} |",
]
...
if ($LASTEXITCODE -ne 0) {
    throw "Synthese de campagne panne/reprise echouee."
}
