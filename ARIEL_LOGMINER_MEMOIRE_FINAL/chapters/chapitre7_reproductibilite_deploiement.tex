...
La campagne Redis d'endurance de six heures a ete lancee depuis un hote Windows avec un environnement Python local. Le service Redis utilise par le script etait le conteneur Docker \texttt{logminer-redis}, base sur l'image \texttt{redis:7-alpine}, expose sur \texttt{localhost:6379}. L'URL Redis effective etait donc \texttt{redis://localhost:6379/0}. Les workers de cette campagne etaient des processus locaux ; les VM Debian/Ubuntu ne constituent pas la preuve de cette campagne de six heures.
